...
\begin{prf}
	...
	\[\tilde\calG(M)_{(e)}\subseteq W'\subseteq W\eqdef\tilde\calG(M)\]
	(that is, that $W$ corresponds to a submodule in $\calG(M)$). Then consider multiplying everything by $e$ above:
	we get
	\[e\tilde\calG(M)=0\subseteq eW'\subseteq eW=e(Se\otimes M)=e\otimes eSeM\cong M\]
	...
	\[W'\supseteq SeW'=S(eW)=S(eSe\otimes_{eSe} M)= Se\otimes eSe M\cong W.\]
	Thus we can conclude that $W'=W$, so $\calG(M)$ has no nontrivial proper submodules, so it is simple.
\end{prf}
...
Let $\calC$ be a symmetric tensor (i.e. monoidal) category with tensor product $\otimes$ and unit object $\1$. After giving some 
...
\begin{defn}
	A \textbf{tensor-triangulated} category $\calC$ is a moniodal category (with $\otimes$ and $\1$) and a triangulated category with 
	distinguished triangles $\calT$ comprised of objects in $\calC$ and shift functor $(-)[1]:\calC\to \calC$ such that:
	\begin{itemize}
		\item $-\otimes-$ is a triangulated (exact) functor in each entry.
		\item 
	\end{itemize}
\end{defn}
