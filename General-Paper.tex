...
Let $V,W$ be (finite dimensional) vector spaces over a field $k$. There are many equivalent formulations of polynomial maps between such spaces, 
...
$\Spec S^\ast(V^\vee)=V\otimes_k-$ (which we, through an abuse of notation, denote again $V$) represented by the symmetric algebra of the dual of $V$. Thus the polynomial maps
are precisely the ones considers between these objects in their algebro-geometric realizations.

\begin{rmk}
	In Friedlander and Suslin's original paper, they define these maps abstractly as $S^\ast(V^\vee)\otimes W$. That this 
	agrees with our definition follows from the following series of isomorphisms:
	\begin{align*}
		\Hom_\text{Pol}(V,W)&\eqdef\Hom_{\Sch/k}(V,W)\\
	...
	\end{align*}
	where we used above properties of affine schemes and standard facts of linear algebra as well as the fact that 
	a map from $S^\ast(V)$ is determined uniquely by its images on $V$.
\end{rmk}
The upshot to Friedlander and Suslin's definition is it avoids problems that arise in the case of infinite-dimensional 
vector spaces as well as enabling us to make more simple the following:
\begin{defn}\label{def:homog-poly-map}
	...
	\[\hat f\in W\otimes S^d(V^\vee).\]
\end{defn}
This is clearly a tangible and sensible way to define a degree $d$ map and it is less obvious how to define a property 
on the map of corresponding varieties that achieves the same goal. We will see in the next subsection a way to narrow our focus using 
the algebro-geometric viewpoint. For now,
\begin{ex}
...
\subsection{The categories \texorpdfstring{$\Gamma^dP_k$}{GdPk} and \texorpdfstring{$\Rep \Gamma^d_k$}{Rep Gdk}}
...
Earlier I promised that we would have a more natural interpretation of the homogeneous degree 
...
This gives us a way to measure the degree of a strict polynomial functor by looking at the representation theory of our 
functor under the natural action by $\Gm$ on vector spaces (by scaling). That this corresponds with our original notion of degree 
for polynomial maps is as follows:
\begin{lem}
	If $f\in\Hom_{\text{Pol}}(V,W)$ is a map homogeneous of degree $n$ (using defn.~\ref{def:homog-poly-map}), then 
	\[f(\lambda \cdot v)=\lambda^n f(v)\]
	for all $\lambda\in k$ and $v\in V$.
\end{lem}
\begin{prf}
	It is important that one identify the actions on everything here first so that we can make the proper identifications. $V$
	has a natural $k^\times$ action by scaling and this extends to an action on $V^\vee$ by 
	\[(\lambda\cdot f)(v)=f(\lambda v)\]
	and to a basis for $S^d(V^\vee)$ by
	\begin{align*}
		\lambda\cdot \left(\sum_{\sigma\in\frakS_d}x_{i_{\sigma(1)}}\otimes\cdots\otimes x_{i_{\sigma(d)}}\right)(v)&=\left(\sum_{\sigma\in\frakS_d}x_{i_{\sigma(1)}}\otimes\cdots\otimes x_{i_{\sigma(d)}}\right)(\lambda v)\\
		&=\sum_{\sigma\in\frakS_d}x_{i_{\sigma(1)}}(\lambda v)\otimes\cdots\otimes x_{i_{\sigma(d)}}(\lambda v)\\
		&=\lambda^d\sum_{\sigma\in\frakS_d}x_{i_{\sigma(1)}}\otimes\cdots\otimes x_{i_{\sigma(d)}}
	\end{align*}
	whence the action on $S^d(V^\vee)$ as a whole is simply 
	\[(\lambda\cdot f)(v)=\lambda^df(v)\]
	which extends linearly to an action of $k^\times$ on all of $S^\ast(V^\vee)$.

	Now by our previous definition of degree, we said a homogeneous degree $n$ polynomial map $V\to W$ was an element 
	of $W\otimes S^n(V^\vee)$. We need to translate this into an honest map of schemes and show that 
	the action described above has the desired effect. If our map is given by the element $\sum_i w_i\otimes f_i$,
	we get our map of representing algebras
	\[f^\ast:S^\ast(W^\vee)\to S^\ast(V^\vee) \quad\text{sending}\quad \rho\mapsto \sum_i\rho(w_i)f_i.\]
	
	{\color{red} This proof needs to be overhauled. I am not even sure this is exactly what I should be proving.
	I may need to go over it again with a fine-toothed comb to trace the actions through exactly. Eventually we will need to 
	extend fields but that should be easy.}
\end{prf}

From this, we see that our notions of degrees agree and we can continue on to define 
\begin{defn}
...
As with any $k$-linear category, we can also define the \textbf{$k$-linear representation category}
\[\Rep\Gamma^d_k\eqdef \Func_k(\Gamma^dP_k,\Vectk),\]
and with this category we can finally connect things to our discussion in section 2:
\begin{thm}
	The map 
	\[\]
\end{thm}
...
Look into and figure out why we need stability here to make sense of things.
